...
function DiffLine({ type, num, text }: { type: DiffType; num: number | null; text: string }) {
  ...
  const bgCls =
    type === "add"  ? "bg-green-400/10" :
    type === "del"  ? "bg-red-400/10" :
    type === "hunk" ? "bg-blue-400/[0.06]" : "";
  const sigCls =
    type === "add"  ? "text-green-400" :
    type === "del"  ? "text-red-400" :
    type === "hunk" ? "text-blue-400 italic" : "text-transparent";
  ...
  return (
    <div className={`flex items-stretch min-h-[22px] hover:bg-white/[0.02] ${bgCls}`}>
      ...
      <span className={`w-5 text-center text-[12px] flex items-center justify-center flex-shrink-0 ${sigCls}`}>
        {sigMap[type]}
      </span>
      <span
      ...
      <div className="w-full max-w-[920px]">
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-[#21262d] rounded-t-xl border border-[#30363d] border-b-0">
          <div className="flex items-center gap-3">
            <span className="text-[13px] font-semibold text-[#e6edf3] font-mono">src/utils/auth.ts</span>
          ...
          {view === "split" ? (
            <div className="flex">
              <div className="flex-1 min-w-0 overflow-x-auto border-r border-[#21262d]">
                <div className="px-0 py-1.5 text-[11px] font-bold text-[#8b949e] uppercase tracking-wide bg-[#161b22] border-b border-[#21262d] pl-4">Before</div>
                <div className="font-mono text-[12.5px] leading-[1.6]">{leftLines}</div>
              </div>
              <div className="flex-1 min-w-0 overflow-x-auto">
                <div className="px-0 py-1.5 text-[11px] font-bold text-[#8b949e] uppercase tracking-wide bg-[#161b22] border-b border-[#21262d] pl-4">After</div>
