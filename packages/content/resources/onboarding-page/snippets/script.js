(function () {
  ...
  function updateProgress() {
    steps.forEach(function (s, i) {
      ...
      if (stepNum === currentStep) {
        ...
        s.querySelector('.ob-step-circle').innerHTML = '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="#fff" stroke-width="2.5"><polyline points="3,8 6.5,11.5 13,5"/></svg>';
      } else {
  ...
  function removeInvite(email) {
    invitedEmails = invitedEmails.filter(function (e) { return e !== email; });
    renderInvites();
  }
  ...
  function renderInvites() {
    ...
    invitedEmails.forEach(function (email) {
      ...
      tag.innerHTML = email + '<button class="ob-invite-remove" data-email="' + email + '">&times;</button>';
  ...
  function buildCompletion() {
    ...
    if (role) {
      ...
      summaryHtml += '<div class="ob-summary-item"><span class="ob-summary-label">Role</span><span class="ob-summary-value">' + roleText + '</span></div>';
    }

    var company = companyInput.value.trim();
    if (company) {
      summaryHtml += '<div class="ob-summary-item"><span class="ob-summary-label">Company</span><span class="ob-summary-value">' + company + '</span></div>';
    }
    ...
    if (selectedTopics.size > 0) {
      ...
      summaryHtml += '<div class="ob-summary-item"><span class="ob-summary-label">Interests</span><span class="ob-summary-value">' + topicLabels.join(', ') + '</span></div>';
    }

    var freqText = frequencySelect.options[frequencySelect.selectedIndex].text;
    summaryHtml += '<div class="ob-summary-item"><span class="ob-summary-label">Email</span><span class="ob-summary-value">' + freqText + '</span></div>';

    if (invitedEmails.length > 0) {
      summaryHtml += '<div class="ob-summary-item"><span class="ob-summary-label">Invites sent</span><span class="ob-summary-value">' + invitedEmails.length + ' teammate' + (invitedEmails.length > 1 ? 's' : '') + '</span></div>';
    }
  ...
  function launchConfetti() {
    ...
    var colors = ['#6366f1', '#8b5cf6', '#ec4899', '#f59e0b', '#22c55e', '#3b82f6', '#ef4444', '#14b8a6'];
