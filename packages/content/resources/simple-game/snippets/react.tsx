...
export default function SimpleGameRC() {
  ...
  function startGame() {
    ...
    s.snake = [{ x: 10, y: 10 }];
    s.dx = 1; s.dy = 0;
    s.score = 0; s.running = true; s.gameOver = false;
    s.food = placeFood(s.snake);
    setScore(0);
    setPhase("running");
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      const s = stateRef.current;
      if (!s.running && e.code === "Space") { startGame(); return; }
      switch (e.key) {
        case "ArrowUp": case "w": if (s.dy === 0) { s.dx = 0; s.dy = -1; } break;
        case "ArrowDown": case "s": if (s.dy === 0) { s.dx = 0; s.dy = 1; } break;
        case "ArrowLeft": case "a": if (s.dx === 0) { s.dx = -1; s.dy = 0; } break;
        case "ArrowRight": case "d": if (s.dx === 0) { s.dx = 1; s.dy = 0; } break;
      }
    }
    window.addEventListener("keydown", onKey);
    return () => { window.removeEventListener("keydown", onKey); if (loopRef.current) clearInterval(loopRef.current); };
  }, []);

  return (
    <div className="min-h-screen bg-[#0d1117] flex flex-col items-center justify-center gap-4 p-6">
      <div className="flex gap-8 text-sm">
        <span className="text-[#8b949e]">Score: <strong className="text-[#34d399] tabular-nums">{score}</strong></span>
        <span className="text-[#8b949e]">Best: <strong className="text-[#f1e05a] tabular-nums">{highScore}</strong></span>
      </div>
      ...
      <div className="relative rounded-xl overflow-hidden border border-[#30363d]">
        ...
        {phase !== "running" && (
          <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center gap-4">
            <p className="text-[#e6edf3] text-xl font-bold">{phase === "over" ? "Game Over" : "Snake"}</p>
            {phase === "over" && <p className="text-[#8b949e] text-sm">Score: {score}</p>}
            <button onClick={startGame} className="px-6 py-2.5 bg-[#238636] border border-[#2ea043] text-white rounded-xl font-semibold text-sm hover:bg-[#2ea043] transition-colors">
      ...
      <div className="grid grid-cols-3 gap-1 mt-2">
        {[
          { label: "↑", row: 1, col: 2, dir: { dx: 0, dy: -1 }, cond: (s: typeof stateRef.current) => s.dy === 0 },
          { label: "←", row: 2, col: 1, dir: { dx: -1, dy: 0 }, cond: (s: typeof stateRef.current) => s.dx === 0 },
          { label: "↓", row: 2, col: 2, dir: { dx: 0, dy: 1 }, cond: (s: typeof stateRef.current) => s.dy === 0 },
          { label: "→", row: 2, col: 3, dir: { dx: 1, dy: 0 }, cond: (s: typeof stateRef.current) => s.dx === 0 },
        ].map(({ label, row, col, dir, cond }) => (
          <button
            key={label}
            onClick={() => { const s = stateRef.current; if (s.running && cond(s)) { s.dx = dir.dx; s.dy = dir.dy; } }}
