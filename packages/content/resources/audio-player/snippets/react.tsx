...
export default function AudioPlayerRC() {
  ...
  function prev() { goTo((trackIdx - 1 + PLAYLIST.length) % PLAYLIST.length); }
  function next() { goTo((trackIdx + 1) % PLAYLIST.length); }

  return (
    <div className="min-h-screen bg-[#0d1117] flex items-center justify-center p-6">
      <div className="w-full max-w-sm bg-[#161b22] border border-[#30363d] rounded-2xl overflow-hidden">
        {/* Album art area */}
        <div className="h-40 flex items-center justify-center" style={{ background: `${track.color}18` }}>
          <div
            className="w-20 h-20 rounded-2xl flex items-center justify-center text-4xl shadow-lg"
            style={{ background: track.color, transform: playing ? "rotate(5deg)" : "rotate(0deg)", transition: "transform 0.3s" }}
          >
            🎵
          </div>
        </div>
        ...
        <div className="p-5">
          ...
          <div className="flex items-center justify-center gap-6 mb-4">
            <button onClick={prev} className="text-[#8b949e] hover:text-[#e6edf3] transition-colors">
            ...
            <button onClick={togglePlay} className="w-12 h-12 rounded-full flex items-center justify-center transition-all" style={{ background: track.color }}>
              {playing
            ...
            <button onClick={next} className="text-[#8b949e] hover:text-[#e6edf3] transition-colors">
          ...
          <div className="flex items-center gap-2">
            ...
            <input type="range" min={0} max={1} step={0.05} value={volume} onChange={(e) => { setVolume(Number(e.target.value)); if (audioRef.current) audioRef.current.volume = Number(e.target.value); }} className="flex-1 h-1 accent-[#8b949e] cursor-pointer" />
            <span className="text-xs">🔊</span>
          </div>
        </div>
        ...
        <div className="border-t border-[#30363d] divide-y divide-[#21262d]">
          {PLAYLIST.map((t, i) => (
            ...
            >
              <div className="w-7 h-7 rounded-lg flex items-center justify-center text-sm flex-shrink-0" style={{ background: `${t.color}30` }}>
                {i === trackIdx && playing ? "▶" : <span className="text-[#484f58] text-xs">{i + 1}</span>}
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-sm truncate ${i === trackIdx ? "text-[#e6edf3] font-semibold" : "text-[#8b949e]"}`}>{t.title}</p>
