const CITATIONS = {
  ...
  3: {
    ...
    domain: 'arxiv.org',
    excerpt: 'We explore a general-purpose fine-tuning recipe for retrieval-augmented generation (RAG), combining parametric and non-parametric memory for language generation.',
    url: '#',
  },
  4: {
    title: 'RAG vs Fine-Tuning: Enterprise Evaluation',
    domain: 'research.google.com',
    excerpt: 'In enterprise deployments, RAG consistently outperforms fine-tuning for tasks requiring frequently updated domain knowledge, while maintaining lower operational costs.',
    url: '#',
  },
  5: {
    title: 'Constitutional AI: Harmlessness from AI Feedback',
    domain: 'anthropic.com',
    excerpt: 'We propose a method for training a harmless AI assistant without any human labels identifying harmful outputs, using a set of principles to guide revisions during supervised learning and RL.',
    url: '#',
  },
};
...
function hideTooltip() {
  tooltip.hidden = true;
  if (activeCit) { activeCit.classList.remove('active'); activeCit = null; }
}
...
document.querySelectorAll('.citation').forEach(cit => {
  ...
  cit.addEventListener('blur', () => {
    setTimeout(() => { if (!tooltip.contains(document.activeElement)) hideTooltip(); }, 100);
  });
});
