...
const DAYS = Array.from({length:14},(_,i)=>{const d=new Date();d.setDate(d.getDate()-13+i);return d.toLocaleDateString('en',{month:'short',day:'numeric'});});
...
function draw() {
  ...
  // Crosshair
  const ch=el('line',{class:'crosshair',id:'crosshair',x1:0,x2:0,y1:PAD.top,y2:PAD.top+cH});
  svg.appendChild(ch);
  ...
  SERIES.forEach((s,si)=>{
    ...
    const line=el('polyline',{class:`line-path line-path-animated`,points:linePts,stroke:s.color,style:`animation-delay:${si*0.15}s`});
    svg.appendChild(line);
    pts.forEach(([px,py],i)=>{
      const dot=el('circle',{cx:px,cy:py,r:4,fill:s.color,stroke:'var(--surface)',
'stroke-width':2,style:'opacity:0;transition:opacity .15s;cursor:pointer','data-si':si,'data-i':i});
      svg.appendChild(dot);
    });
  });
  ...
  svg.addEventListener('mousemove',(e)=>{
    ...
    if(idx<0||idx>=n){tooltip.hidden=true;return;}
    const x=xOf(idx);
