let comments = [
  {
    id: 1, name: 'Sarah Kim', color: '#0ea5e9', text: 'This is exactly what I needed! Copying the sidebar component right now 🚀', time: new Date(Date.now() - 3600000 * 2), likes: 14, liked: false,
    replies: [
      { id: 11, name: 'Marcus Reed', color: '#8b5cf6', text: 'Same here — saved me hours of work.', time: new Date(Date.now() - 3600000), likes: 5, liked: false, replies: [] }
    ]
  },
  {
    id: 2, name: 'Julia Lee', color: '#f59e0b', text: 'The breadcrumb with SEO structured data injection is a really nice touch. Love the attention to detail.', time: new Date(Date.now() - 86400000), likes: 8, liked: false, replies: []
  },
  {
    id: 3, name: 'David Okonkwo', color: '#34d399', text: 'Is there a React version of the sidebar component planned?', time: new Date(Date.now() - 86400000 * 2), likes: 3, liked: false, replies: []
  }
];
let nextId = 100;
...
function buildComment(c, isReply = false) {
  ...
  el.innerHTML = `
    <div class="comment-header">
      <div class="comment-av" style="background:${c.color};width:32px;height:32px;border-radius:50%;display:grid;place-items:center;font-size:0.65rem;font-weight:700;color:#fff;flex-shrink:0">${c.name.split(' ').map(n=>n[0]).join('')}</div>
    ...
    ${repliesHTML}
    ${!isReply ? `<div class="inline-reply-form" id="replyForm-${c.id}" hidden>
...
function renderComments() {
  ...
  document.getElementById('commentsCount').textContent = comments.reduce((a, c) => a + 1 + (c.replies?.length || 0), 0);
  ...
  list.querySelectorAll('.submit-reply-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (c) {
        c.replies.push({ id: nextId++, name: 'You', color: '#818cf8', text: ta.value.trim(), time: new Date(), likes: 0, liked: false, replies: [] });
        renderComments();
      }
    });
  });
...
topForm?.addEventListener('submit', (e) => {
  ...
  if (!txt) return;
  comments.unshift({ id: nextId++, name: 'You', color: '#818cf8', text: txt, time: new Date(), likes: 0, liked: false, replies: [] });
  topTA.value = '';
