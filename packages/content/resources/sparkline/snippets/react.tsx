...
const ROWS = [
  { label:"Revenue",        values:[3200,2900,3800,4100,3700,4500,4900,4200,5100,4700,5300,5800], color:"#818cf8", suffix:"k" },
  { label:"Active Users",   values:[810,770,840,920,880,960,1010,980,1050,990,1080,1120],          color:"#34d399", suffix:"" },
  { label:"Conversion",     values:[2.1,2.4,2.2,2.8,2.6,3.1,2.9,3.3,3.0,3.5,3.2,3.8],            color:"#f59e0b", suffix:"%" },
  { label:"Bounce Rate",    values:[54,58,52,49,55,47,50,46,48,45,43,41],                          color:"#f87171", suffix:"%" },
  { label:"Avg. Session",   values:[2.1,2.3,2.0,2.4,2.2,2.6,2.5,2.8,2.7,3.0,2.9,3.2],            color:"#a78bfa", suffix:"m" },
];
    ...
    <div className="min-h-screen bg-[#0d1117] p-6 flex justify-center">
      <div className="w-full max-w-[600px] bg-[#0d1117] rounded-xl border border-[#21262d] overflow-hidden">
        <div className="grid grid-cols-[1fr_auto_auto_auto] px-4 py-2 border-b border-[#21262d] text-[11px] text-[#484f58] font-semibold uppercase tracking-wider">
          <span>Metric</span><span className="text-right pr-6">Trend</span><span className="text-right pr-4">Current</span><span className="text-right">Change</span>
        </div>
        {ROWS.map(row => {
          ...
          return (
            <div key={row.label} className="grid grid-cols-[1fr_auto_auto_auto] items-center px-4 py-3 border-b border-[#21262d] last:border-0 hover:bg-[#161b22] transition-colors">
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background:row.color }}/>
                <span className="text-[#8b949e] text-[13px]">{row.label}</span>
              </div>
              <div className="pr-6"><Sparkline values={row.values} color={row.color}/></div>
              <div className="text-[#e6edf3] font-semibold text-[14px] pr-4">{fmt(cur)}</div>
              <div className={`text-[12px] font-semibold text-right ${isUp?"text-[#34d399]":"text-[#f87171]"}`}>
                {isUp?"▲":"▼"}{Math.abs(+delta)}%
              </div>
            </div>
          );
