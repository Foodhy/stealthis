...
// Ears
[-1, 1].forEach(s => {
  const ear = new THREE.Mesh(new THREE.SphereGeometry(0.50, 12, 12), skinMat);
...
function buildBrow(xSide) {
  ...
  brow.position.set(xSide * 0.72, 0.90, 1.76);
  brow.rotation.z = xSide * -0.10;
  headGroup.add(brow);
  return brow;
}
...
// ─── Blush ───────────────────────────────────────────────────────────────────
[-1.05, 1.05].forEach(x => {
  const blush = new THREE.Mesh(new THREE.CircleGeometry(0.40, 32), blushMat);
  blush.position.set(x, -0.30, 1.80);
  headGroup.add(blush);
});

// ─── Smile ───────────────────────────────────────────────────────────────────
const smile = new THREE.Mesh(
  new THREE.TorusGeometry(0.52, 0.065, 8, 26, Math.PI * 0.72),
  smileMat
);
smile.position.set(0.10, -0.80, 1.88);
smile.rotation.z = Math.PI * 0.14;
headGroup.add(smile);

// ─── Neck + Shoulders (static, not in headGroup) ─────────────────────────────
const neck = new THREE.Mesh(
  new THREE.CylinderGeometry(0.55, 0.68, 0.85, 16),
  skinMat
);
...
const shirt = new THREE.Mesh(
  new THREE.CapsuleGeometry(1.05, 2.0, 8, 16),
  shirtMat
);
...
const collar = new THREE.Mesh(
  new THREE.CylinderGeometry(0.70, 0.72, 0.35, 16),
...
(function animate() {
  ...
  const targetRotY =  Math.atan2(cursor3D.x, cursor3D.z + 3) * 0.30;
  ...
  leftBrow.position.y  = 0.90 + browLift;
  rightBrow.position.y = 0.90 + browLift;
