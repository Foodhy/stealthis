...
const SIZE=360, CX=SIZE/2, CY=SIZE/2+10, R=130, RINGS=5;
...
function draw(){
  ...
  SERIES.forEach((s,si)=>{
    ...
    const poly=elSvg('polygon',{points:pts,class:'radar-polygon',fill:s.color,stroke:s.color,
      style:`animation:fadeIn .5s ${si*0.15}s ease both`});
    svg.appendChild(poly);
    s.data.forEach((v,i)=>{
      const [dx,dy]=polarXY(CX,CY,(v/100)*R,i*step);
      const dot=elSvg('circle',{class:'radar-dot',cx:dx,cy:dy,fill:s.color,
        stroke:'var(--surface)','stroke-width':2,'data-si':si,'data-i':i});
