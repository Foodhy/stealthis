...
const SIZE = 320, CX = SIZE/2, CY = SIZE/2+10, R = 120, RINGS = 5;
    ...
    <div className="min-h-screen bg-[#0d1117] p-6 flex justify-center">
        ...
        <div className="flex justify-center">
          <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
            ...
            {Array.from({length:RINGS},(_,r) => {
              ...
              return (
                <g key={r}>
                  ...
                  <text x={polarXY((R/RINGS)*(r+1),0)[0]+3} y={polarXY((R/RINGS)*(r+1),0)[1]} fill="#484f58" fontSize={9}>{Math.round((100/RINGS)*(r+1))}</text>
                </g>
              );
            })}
            {/* Axes + labels */}
            {AXES.map((axis,i) => {
              ...
              return (
                <g key={axis}>
                  ...
                  <text x={lx} y={ly+4} textAnchor={anchor} fill="#8b949e" fontSize={11}>{axis}</text>
                </g>
              );
            })}
            {/* Series */}
            {SERIES.map((s,si) => {
              ...
              return (
                <g key={s.label}>
                  <polygon points={ptsStr(pts)} fill={s.color} fillOpacity={0.15} stroke={s.color} strokeWidth={2}/>
                  {pts.map(([px,py],i) => (
                    <circle key={i} cx={px} cy={py} r={4} fill={s.color} stroke="#0d1117" strokeWidth={2} style={{ cursor:"pointer" }}
                      onMouseEnter={e => setTooltip({ axis:AXES[i], label:s.label, value:s.data[i], x:e.clientX, y:e.clientY })}
      ...
      {tooltip && (
        <div className="fixed pointer-events-none bg-[#161b22] border border-[#30363d] rounded-lg px-3 py-2 text-[12px] shadow-lg z-50"
          ...
          <div className="text-[#8b949e]">{tooltip.label}: <strong className="text-[#e6edf3]">{tooltip.value}</strong></div>
        </div>
      )}
    </div>
  );
