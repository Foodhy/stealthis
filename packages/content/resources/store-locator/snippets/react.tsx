...
const STORES: Store[] = [
  { id: 1, name: "Flagship — Union Square",      address: "170 O'Farrell St",       city: "San Francisco",  state: "CA", lat: 37.7869, lng: -122.4072, distance: "0.4 mi",  distanceNum: 0.4,  open: true,  hours: "Mon–Sat 9–9, Sun 11–7",  phone: "(415) 555-0101", rating: 4.8 },
  { id: 2, name: "SoMa Outlet",                  address: "899 Howard St",           city: "San Francisco",  state: "CA", lat: 37.7793, lng: -122.4024, distance: "1.2 mi",  distanceNum: 1.2,  open: true,  hours: "Mon–Fri 10–7, Sat–Sun 10–6", phone: "(415) 555-0102", rating: 4.3 },
  { id: 3, name: "Mission District",             address: "2401 Mission St",         city: "San Francisco",  state: "CA", lat: 37.7568, lng: -122.4189, distance: "2.8 mi",  distanceNum: 2.8,  open: false, hours: "Tue–Sun 10–7",             phone: "(415) 555-0103", rating: 4.6 },
  { id: 4, name: "Berkeley Marina",              address: "225 University Ave",      city: "Berkeley",       state: "CA", lat: 37.8702, lng: -122.2679, distance: "5.5 mi",  distanceNum: 5.5,  open: true,  hours: "Daily 10–8",               phone: "(510) 555-0104", rating: 4.7 },
  { id: 5, name: "Oakland City Center",          address: "20th & Broadway",         city: "Oakland",        state: "CA", lat: 37.8083, lng: -122.2712, distance: "8.1 mi",  distanceNum: 8.1,  open: true,  hours: "Mon–Sat 10–8, Sun 11–6",   phone: "(510) 555-0105", rating: 4.4 },
  { id: 6, name: "Palo Alto",                    address: "340 University Ave",      city: "Palo Alto",      state: "CA", lat: 37.4459, lng: -122.1613, distance: "29 mi",   distanceNum: 29,   open: false, hours: "Mon–Sat 10–7",             phone: "(650) 555-0106", rating: 4.5 },
];

// Pseudo-map using SVG with real-ish coordinate mapping
function MapView({ stores, selectedId, onSelect }: {
  stores: Store[];
  selectedId: number | null;
  onSelect: (id: number) => void;
}) {
  // Map bounds (roughly Bay Area)
  const minLat = 37.4, maxLat = 37.93;
  const minLng = -122.52, maxLng = -122.1;
    ...
    <div className="relative w-full h-full bg-[#0d1117] rounded-xl overflow-hidden border border-[#30363d]">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-full" preserveAspectRatio="xMidYMid meet">
        ...
        <path
          ...
          fill="#1c2128" stroke="#30363d" strokeWidth="1" opacity="0.7"
        />
        <text x="460" y="190" fill="#484f58" fontSize="10" fontFamily="monospace">San Francisco Bay</text>
        ...
        {[
          ...
          <path key={i} d={d} fill="none" stroke="#21262d" strokeWidth={i === 0 || i === 1 ? 3 : 1.5}/>
        ))}
        ...
        <g transform="translate(28, 28)">
          ...
          <text textAnchor="middle" y="-5" fill="#8b949e" fontSize="10" fontFamily="monospace">N</text>
...
export default function StoreLocatorRC() {
  ...
  const filtered = STORES
    ...
    .sort((a, b) =>
      sortBy === "distance" ? a.distanceNum - b.distanceNum : b.rating - a.rating
    );
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <p className={`text-[12px] font-semibold leading-tight ${isSelected ? "text-[#58a6ff]" : "text-[#e6edf3]"}`}>
                      {store.name}
                    </p>
                  ...
                  <p className="text-[11px] text-[#8b949e] truncate">{store.address}, {store.city}</p>
            ...
            {selected && (
              <div className="flex items-center gap-3 bg-[#161b22] border border-[#30363d] rounded-xl px-3 py-2">
                <div className="flex-1 min-w-0">
                  <p className="text-[12px] font-semibold text-[#e6edf3] truncate">{selected.name}</p>
                  <p className="text-[10px] text-[#8b949e]">{selected.hours} · {selected.phone}</p>
                </div>
