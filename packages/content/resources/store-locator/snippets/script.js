const STORES = [
  { id: 1, name: 'Acme — Manhattan', addr: '350 5th Ave, New York, NY', dist: '0.4 km', open: true, flagship: true,  x: 55, y: 42 },
  { id: 2, name: 'Acme — SoHo',      addr: '120 Spring St, New York, NY', dist: '1.8 km', open: true, flagship: false, x: 48, y: 60 },
  { id: 3, name: 'Acme — Brooklyn',  addr: '1 Court Square, LIC, NY', dist: '3.2 km', open: false, flagship: false, x: 72, y: 65 },
  { id: 4, name: 'Acme — Hoboken',   addr: '88 River St, Hoboken, NJ', dist: '5.1 km', open: true, flagship: false, x: 28, y: 50 },
  { id: 5, name: 'Acme — Upper West',addr: '2109 Broadway, New York, NY', dist: '6.3 km', open: true, flagship: false, x: 50, y: 28 },
];
...
function renderList() {
  ...
  STORES.forEach(s => {
    ...
    const tags = [s.open ? '<span class="sl-tag sl-tag--open">Open now</span>' : '', s.flagship ? '<span class="sl-tag sl-tag--flagship">Flagship</span>' : ''].filter(Boolean).join('');
    div.innerHTML = `
...
function filterMatch(s) {
  if (searchQ && !s.name.toLowerCase().includes(searchQ) && !s.addr.toLowerCase().includes(searchQ)) return false;
