...
function level(v: number) { if(v===0)return 0; if(v<2)return 1; if(v<5)return 2; if(v<9)return 3; return 4; }
...
export default function ChartHeatmapRC() {
  ...
  weeks.forEach((week,wi) => {
    ...
    if (m !== prevMonth) { prevMonth=m; monthLabels.push({ month:m, wi }); }
  });
      ...
      <div className="w-full max-w-[900px] mx-auto overflow-x-auto">
        <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ minWidth: W }}>
          ...
          {DAY_LABELS.map((lbl,i) => lbl && (
            <text key={i} x={LEFT_PAD-4} y={TOP_PAD+i*STEP+CELL-1} textAnchor="end" fill="#484f58" fontSize={9}>{lbl}</text>
          ))}
          {/* Month labels */}
          {monthLabels.map(({ month, wi }) => (
            <text key={month} x={LEFT_PAD+wi*STEP} y={TOP_PAD-5} fill="#484f58" fontSize={9}>{MONTHS[month]}</text>
          ))}
          {/* Cells */}
            ...
            return (
              <rect key={`${wi}-${di}`}
                ...
                onMouseEnter={e => {
                  ...
                  setTooltip({ text:`${fmt} — ${day.val} contribution${day.val!==1?"s":""}`, x:e.clientX, y:e.clientY });
                }}
