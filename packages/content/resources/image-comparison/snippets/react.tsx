...
function CompareSlider({ pair }: { pair: typeof PAIRS[0] }) {
  ...
  function onMouseMove(e: React.MouseEvent) { if (dragging) updatePos(e.clientX); }
    ...
    return (
      <div className="w-full h-full flex flex-col p-4 gap-3" style={{ background: side.bg }}>
        <div className="flex items-center gap-2">
          {side.items.map((c, i) => (
            <div key={i} className="h-2 rounded-full" style={{ background: c, width: `${[40, 25, 35][i]}%` }} />
          ))}
        </div>
        <div className="grid grid-cols-2 gap-2 flex-1">
          {side.items.map((c, i) => (
            <div key={i} className="rounded-lg border border-white/10 p-2 flex items-end" style={{ background: `${c}20` }}>
              <div className="h-1.5 rounded-full w-full" style={{ background: c, opacity: 0.7 }} />
            </div>
          ))}
        </div>
        <div className="h-1 rounded-full w-3/4" style={{ background: side.items[0], opacity: 0.4 }} />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      >
        ...
        {/* Labels */}
        <span className="absolute top-2 left-2 text-[10px] bg-black/50 text-white px-1.5 py-0.5 rounded">{pair.before.label}</span>
        <span className="absolute top-2 right-2 text-[10px] bg-black/50 text-white px-1.5 py-0.5 rounded">{pair.after.label}</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] flex items-center justify-center p-6">
      <div className="w-full max-w-sm space-y-5">
        ...
        {PAIRS.map((pair) => <CompareSlider key={pair.label} pair={pair} />)}
