  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`}>
        ...
        <text x={cx} y={cy+4} textAnchor="middle" fill="#e6edf3" fontSize={20} fontWeight={800}>{current}%</text>
        <text x={cx} y={cy+18} textAnchor="middle" fill="#484f58" fontSize={10}>{label}</text>
      </svg>
    </div>
  );
}
