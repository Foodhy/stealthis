...
const SERIES = [
  { label: "Revenue", color: "#818cf8", data: [42,58,51,67,73,89,95,88,102,115,108,127] },
      ...
      <div ref={wrapRef} className="w-full max-w-[800px] mx-auto">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
          <div className="flex gap-2 flex-wrap">
            {SERIES.map((s, i) => (
              <button key={s.label} onClick={() => toggleHidden(i)}
          ...
          <div className="flex gap-1">
            {["Linear","Smooth"].map(m => (
              <button key={m} onClick={() => setSmooth(m==="Smooth")}
        ...
        <div className="relative">
          <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} className="w-full"
            ...
            {/* Grid */}
            {Array.from({length:6},(_,t) => { const v=(maxVal/5)*t; const y=yOf(v); return (
              <g key={t}>
                <line x1={PAD.left} x2={PAD.left+cW} y1={y} y2={y} stroke="#21262d" strokeWidth={1}/>
                <text x={PAD.left-6} y={y+3.5} textAnchor="end" fill="#484f58" fontSize={10}>{Math.round(v)}k</text>
              </g>
            ); })}
            {LABELS.map((lbl,i) => (
              <text key={lbl} x={xOf(i)} y={H-6} textAnchor="middle" fill="#484f58" fontSize={10}>{lbl}</text>
            ))}
            ...
            {SERIES.map((s,si) => {
              ...
              return (
                <g key={s.label}>
                  <path d={smooth?bezierPath(pts):linePath(pts)} fill="none" stroke={s.color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"/>
                  {pts.map(([px,py],i) => (
                    <circle key={i} cx={px} cy={py} r={tooltip?.idx===i?4:2.5} fill={s.color} opacity={tooltip?.idx===i?1:0.7}/>
                  ))}
                </g>
              );
            })}
          ...
          {tooltip && (
            <div className="absolute pointer-events-none bg-[#161b22] border border-[#30363d] rounded-lg px-3 py-2 text-[12px] shadow-lg min-w-[140px]"
