...
const QUESTIONS = [
  ...
  },
  {
    ...
    answers: ["Hyper Text Markup Language", "Home Tool Markup Language", "Hyperlinks and Text Markup Language", "Hyper Text Modern Language"],
    correct: 0,
  },
  {
...
export default function QuizWidgetRC() {
  ...
  function pick(i: number) {
    ...
    setTimeout(() => {
      if (idx + 1 < QUESTIONS.length) { setIdx((n) => n + 1); setSelected(null); }
      else setDone(true);
    }, 1200);
  }
  ...
  function btnClass(i: number) {
    if (selected === null) return "bg-[#21262d] border-[#30363d] text-[#e6edf3] hover:border-[#58a6ff]/60";
    if (i === q.correct) return "bg-[#238636]/30 border-[#7ee787]/50 text-[#7ee787]";
    if (i === selected) return "bg-[#f85149]/20 border-[#f85149]/50 text-[#f85149]";
    return "bg-[#21262d] border-[#30363d] text-[#484f58]";
  }
    ...
    return (
      <div className="min-h-screen bg-[#0d1117] flex items-center justify-center p-6">
        <div className="bg-[#161b22] border border-[#30363d] rounded-2xl p-8 text-center w-full max-w-sm">
          ...
          <h2 className="text-[#e6edf3] text-2xl font-bold mb-1">{score}/{QUESTIONS.length}</h2>
          <p className="text-[#8b949e] text-sm mb-6">You scored {pct}%</p>
          <div className="h-2 bg-[#21262d] rounded-full mb-6">
            <div className="h-full bg-[#58a6ff] rounded-full transition-all" style={{ width: `${pct}%` }} />
          </div>
          <button onClick={restart} className="w-full py-2.5 bg-[#238636] border border-[#2ea043] text-white rounded-xl font-semibold text-sm hover:bg-[#2ea043] transition-colors">
            Try Again
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#0d1117] flex items-center justify-center p-6">
      <div className="bg-[#161b22] border border-[#30363d] rounded-2xl p-6 w-full max-w-sm">
        <div className="flex items-center justify-between mb-4">
          <span className="text-xs text-[#8b949e]">Question {idx + 1}/{QUESTIONS.length}</span>
          <span className="text-xs font-semibold text-[#58a6ff]">Score: {score}</span>
        </div>
        <div className="h-1 bg-[#21262d] rounded-full mb-5">
          <div className="h-full bg-[#58a6ff] rounded-full transition-all" style={{ width: `${((idx) / QUESTIONS.length) * 100}%` }} />
        </div>
        ...
        <div className="space-y-2">
          {q.answers.map((a, i) => (
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-xl border text-sm transition-all duration-200 ${btnClass(i)}`}
            >
              {a}
            </button>
          ))}
