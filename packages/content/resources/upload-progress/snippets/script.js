...
function renderItems() {
  ...
  items.forEach(item => {
    ...
    const pctStr = item.status === 'done' ? '100%' : item.status === 'error' ? 'Failed' : `${item.pct}%`;
    const fillCls = item.status === 'done' ? 'up-bar-fill--done' : item.status === 'error' ? 'up-bar-fill--error' : '';
    const statusCls = item.status === 'done' ? 'up-item-status--done' : item.status === 'error' ? 'up-item-status--error' : '';
    const statusText = item.status === 'done' ? '✓ Complete' : item.status === 'error' ? '✕ Failed' : item.status === 'pending' ? 'Waiting…' : 'Uploading…';
