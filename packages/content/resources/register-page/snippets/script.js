...
document.getElementById('registerForm')?.addEventListener('submit', async (e) => {
  ...
  if (!document.getElementById('firstName').value.trim()) {
    showError('firstName-error', 'First name is required.'); valid = false;
  ...
  if (pwdEl.value.length < 8) {
    showError('password-error', 'Password must be at least 8 characters.'); valid = false;
  } else clearError('password-error');

  if (confirmEl.value !== pwdEl.value) {
    showError('confirm-error', 'Passwords do not match.'); valid = false;
  }

  if (!document.getElementById('terms').checked) {
    showError('terms-error', 'You must accept the terms.'); valid = false;
