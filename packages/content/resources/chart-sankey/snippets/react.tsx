...
type SNode = { id: number; name: string; color: string; val: string; change?: string; logo?: string; x: number; y: number; h: number };
...
const RAW_LINKS = [
  {s:0,t:6,v:48.5},{s:1,t:6,v:8.7},{s:2,t:6,v:7.4},{s:6,t:7,v:64.6},{s:3,t:7,v:9.3},
  {s:4,t:7,v:10.3},{s:5,t:7,v:0.5},{s:7,t:8,v:49.2},{s:7,t:9,v:35.5},{s:8,t:10,v:27.4},
  {s:8,t:11,v:21.8},{s:10,t:12,v:23.6},{s:10,t:13,v:3.7},{s:10,t:14,v:0.1},
  {s:11,t:15,v:11.9},{s:11,t:16,v:6.8},{s:11,t:17,v:3.1},
];
...
function buildLayout(W: number, H: number) {
  ...
  const links: SLink[] = RAW_LINKS.map(l => ({
    source:l.s, target:l.t, value:l.v, color:nodes[l.s]?.color||"#818cf8",
  }));
...
export default function ChartSankeyRC() {
  ...
  nodes.forEach(n => { srcOffset[n.id]=0; tgtOffset[n.id]=0; });

  const renderedLinks = links.map(l => {
    const src=nodes[l.source], tgt=nodes[l.target];
    if(!src||!tgt) return null;
    const totalOut=links.filter(ll=>ll.source===l.source).reduce((a,ll)=>a+ll.value,0);
    const totalIn=links.filter(ll=>ll.target===l.target).reduce((a,ll)=>a+ll.value,0);
    ...
    const sx = src.x+NW, tx = tgt.x;
    const cx = (sx+tx)/2;
    return { path:`M${sx},${sy-lh/2} C${cx},${sy-lh/2} ${cx},${ty-lh2/2} ${tx},${ty-lh2/2} L${tx},${ty+lh2/2} C${cx},${ty+lh2/2} ${cx},${sy+lh/2} ${sx},${sy+lh/2} Z`,
      color:l.color, key:`${l.source}-${l.target}`, label:`${nodes[l.source].name} → ${nodes[l.target].name}: $${l.value}B` };
  }).filter(Boolean);
    ...
    <div className="min-h-screen bg-[#0d1117] p-4 overflow-x-auto">
      <div style={{ minWidth:W }}>
        <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} className="w-full">
          {/* Links */}
          {renderedLinks.map(l => l && (
            <path key={l.key} d={l.path} fill={l.color} fillOpacity={hovered===null?0.18:0.08}
              style={{ cursor:"pointer", transition:"fill-opacity 0.15s" }}
              onMouseEnter={e => { setTooltip({ text:l.label, x:e.clientX, y:e.clientY }); }}
          ...
          {nodes.map((n,i) => n && (
            <g key={i} onMouseEnter={()=>setHovered(i)} onMouseLeave={()=>setHovered(null)}>
              <rect x={n.x} y={n.y} width={NW} height={n.h} rx={3} fill={n.color}
                opacity={hovered===null||hovered===i?1:0.5} style={{ transition:"opacity 0.15s" }}/>
              <text x={n.x>W/2?n.x-6:n.x+NW+6} y={n.y+n.h/2-4}
                textAnchor={n.x>W/2?"end":"start"} fill="#e6edf3" fontSize={9} fontWeight={600}>{n.name}</text>
              <text x={n.x>W/2?n.x-6:n.x+NW+6} y={n.y+n.h/2+7}
                textAnchor={n.x>W/2?"end":"start"} fill={n.color} fontSize={10} fontWeight={700}>{n.val}</text>
              {n.change&&<text x={n.x>W/2?n.x-6:n.x+NW+6} y={n.y+n.h/2+19}
                textAnchor={n.x>W/2?"end":"start"} fill="#484f58" fontSize={9}>{n.change}</text>}
            </g>
          ))}
        </svg>
      </div>
