...
### Snippet authoring — dismissible layers (modals, drawers, toasts, menus)

> ⚠️ **Este es el bug #1 de los snippets del repo.** Auditoría 2026-07-30: **203 de 2127 recursos**
> tenían un modal/menú/toast que aparecía abierto y **no se podía cerrar**. Léelo antes de escribir
> cualquier overlay en `packages/content/resources/*/snippets/`.

**La causa raíz.** El patrón habitual es marcar el overlay con el atributo `hidden` en el HTML y
alternarlo desde JS (`el.hidden = false`). Pero el atributo `hidden` solo aplica `display: none`
desde la hoja de estilos del navegador, que tiene **la especificidad más baja posible**. En cuanto
el CSS escribe un `display` sobre ese mismo elemento —y siempre lo hace, porque el overlay necesita
`display: grid` o `flex` para centrarse— **gana el CSS**. Resultado: el modal se ve desde que carga
la página y el botón de cerrar no hace nada visible, porque poner `hidden = true` ya no cambia nada.

```css
/* ❌ El bug. .modal-backdrop pisa el display:none del atributo hidden. */
.modal-backdrop { position: fixed; inset: 0; display: grid; place-items: center; }
```
```html
<div class="modal-backdrop" id="backdrop" hidden> <!-- se ve igual, y no se puede cerrar --> </div>
```

**La regla.** Pon esta línea en el `:root`/reset de **todo** snippet que use `hidden`. Es una línea y
elimina la clase de bug entera, incluso si luego alguien añade un `display` al overlay:

```css
/* ✅ Restaura la semántica de [hidden] por encima de cualquier regla de clase. */
[hidden] { display: none !important; }
```

Alternativa igual de válida: no uses `hidden` y controla la visibilidad con una clase propia
(`.modal.is-open { display: grid }` y sin `display` en la regla base). Lo que **no** vale es mezclar
las dos cosas, que es exactamente lo que produce el fallo.

**Checklist para cualquier capa descartable** (modal, drawer, bottom sheet, lightbox, mega-menu,
toast, popover). Antes de dar por hecho el recurso, verifica los seis puntos:

1. **Arranca cerrada.** Ningún overlay debe estar visible al cargar la página, salvo que el recurso
   trate justamente de eso. Comprueba el estado inicial en el navegador, no solo en el HTML.
2. **Se cierra por al menos tres vías**: botón ✕, click en el backdrop (`if (e.target === backdrop)`),
   y tecla `Escape`. Si hay botón Cancelar, también.
3. **El cierre funciona de verdad** — abre y cierra en el lab antes de terminar. Que el JS ejecute
   `closeModal()` no prueba nada si el CSS lo mantiene visible.
4. **Devuelve el scroll.** Si al abrir pones `document.body.style.overflow = "hidden"`, restáuralo al
   cerrar; si no, la página queda bloqueada aunque el modal desaparezca.
5. **Foco**: mueve el foco dentro al abrir y devuélvelo al disparador al cerrar. `role="dialog"` +
   `aria-modal="true"` en el contenedor.
6. **`Escape` solo actúa si está abierto** (`if (e.key === "Escape" && !backdrop.hidden)`), para no
   pisar otros atajos de la página.

El mismo razonamiento aplica a cualquier elemento con `hidden` que no sea un overlay — mensajes de
error de formulario, estados vacíos, barras de progreso, notas de "descartado". Si el CSS le da un
`display`, el `hidden` deja de servir.
