...
export default function Values() {
  ...
  const handleOpenDialog = (item?: any) => {
    ...
    if (activeTab === "authors") {
      ...
      setFormData(
        item
          ? { ...item, configuration: JSON.stringify(item.configuration || {}, null, 2) }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      toast({
        title: t("values.toast.errorTitle"),
        description: error.message || t("values.toast.saveError"),
        variant: "destructive",
      });
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      toast({
        title: t("values.toast.errorTitle"),
        description: t("values.toast.deleteError"),
        variant: "destructive",
      });
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="sm:max-w-lg h-auto max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editingItem ? t("values.dialog.edit") : t("values.dialog.create")}{" "}
                {itemLabel(activeTab)}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={handleSave} className="space-y-4 mt-4">
                ...
                <>
                  <div className="space-y-2">
                    <label className="text-sm font-medium">
                      {t("values.form.toolInternalName")}
                    </label>
                    <Input
                      value={formData.name || ""}
                      onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              ...
              {activeTab === "variables" && (
                <>
                  <div className="space-y-2">
                    <label className="text-sm font-medium">
                      {t("values.form.variableCategory")}
                    </label>
                    <Input
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("values.form.variableNoBraces")}
                    </p>
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium">
                      {t("values.form.variableDescription")}
                    </label>
                    <Input
                      value={formData.description || ""}
                      onChange={(e) => setFormData({ ...formData, description: e.target.value })}
