...
const NewDataSource = () => {
    ...
    const loadSources = async () => {
        ...
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : 'Error al cargar las fuentes';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : 'Error al guardar la fuente';
            setError(errorMessage);
    ...
    const handleDelete = async (id: number) => {
        if (window.confirm('¿Estás seguro de eliminar esta fuente?')) {
            ...
            try {
                ...
                const errorMessage = err instanceof Error ? err.message : 'Error al eliminar la fuente';
                setError(errorMessage);
            ...
            <div className="min-h-screen bg-background p-6">
                <div className="max-w-5xl mx-auto space-y-8">
                    ...
                    <div className="flex justify-between items-center">
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight text-foreground">Fuentes de Información</h1>
                            <p className="text-muted-foreground mt-1">Gestione sus fuentes de datos</p>
                        </div>
                        <button
                        ...
                        >
                            ...
                            {isFormOpen ? 'Cancelar' : 'Nueva Fuente'}
                        </button>
                    </div>
                    ...
                    <div className="bg-card border rounded-lg p-4 shadow-sm space-y-4">
                                ...
                                >
                                    <SelectTrigger className="h-full min-h-[40px]">
                                        <div className="flex flex-wrap gap-1 pr-6">
                                            {searchTags.length === 0 ? (
                                                <span className="text-muted-foreground font-normal">Filtrar por tags...</span>
                                            ) : (
                        ...
                        {showAdvancedFilters && (
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 pt-2 border-t mt-2 animate-in fade-in slide-in-from-top-2">
                                    ...
                                    >
                                        <SelectTrigger className="h-9">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span>{sortConfig.field === 'created_at' ? 'Fecha' : 'Nombre'}</span>
                                            </div>
                                        </SelectTrigger>
                                        <SelectContent>
                                ...
                                {/* Show Favorites */}
                                <div className="space-y-1">
                                    <label className="text-xs font-medium text-muted-foreground">Favoritos</label>
                                    <button
                    ...
                    {isFormOpen && (
                        <div className="bg-card border rounded-lg shadow-sm p-6 animate-in fade-in slide-in-from-top-4">
                            <h2 className="text-xl font-semibold mb-6">{editingId ? 'Editar Fuente' : 'Agregar Nueva Fuente'}</h2>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <input
                                            ...
                                            placeholder="Ej: Documentación v1"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                El contenido del archivo se guardará como texto.
                                            </p>
                                        </div>
                                        {formData.content && (
                                            <div className="mt-2">
                                                <label className="text-xs font-medium mb-1 block">Vista previa del contenido:</label>
                                                <textarea
                                                    value={formData.content}
                                                    readOnly
                            ...
                            {filteredSources.map((source) => (
                                <div key={source.id} className="group bg-card border rounded-lg p-5 shadow-sm hover:shadow-md transition-all relative">
                                    ...
                                    <button
                                        ...
                                        title={favorites.includes(source.id) ? "Quitar de favoritos" : "Agregar a favoritos"}
                                    >
                                    ...
                                    <div className="flex justify-between items-start mb-3 pr-8">
                                        <div className="flex items-center gap-2 min-w-0 overflow-hidden flex-1 mr-2">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <span className="text-xs text-muted-foreground uppercase tracking-wider block">{source.source_type}</span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    {sources.length === 0 && !isFormOpen && !loading && (
                        <div className="text-center py-20 border-2 border-dashed rounded-lg bg-secondary/5">
                            ...
                            <h3 className="text-lg font-medium text-foreground">No hay fuentes guardadas</h3>
                            <p className="text-sm text-muted-foreground mt-1 mb-4">Comienza agregando tu primera fuente de información.</p>
                            <button
