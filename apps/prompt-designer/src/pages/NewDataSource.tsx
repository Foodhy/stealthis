...
const NewDataSource = () => {
  ...
  const filteredSources = sources
    .filter((source) => {
      ...
      const matchesTags =
        searchTags.length === 0 || searchTags.every((tagId) => sourceTags.includes(tagId));
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 pt-2 border-t mt-2 animate-in fade-in slide-in-from-top-2">
                ...
                <div className="space-y-1">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setSortConfig({ ...sortConfig, field: value as SortField })
                    }
                  >
                    <SelectTrigger className="h-9">
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {sortConfig.field === "created_at"
                            ? t("newSource.sortDate")
                            : t("newSource.sortName")}
                        </span>
                      </div>
                    </SelectTrigger>
                    <SelectContent>
          ...
          {isFormOpen && (
            <div className="bg-card border rounded-lg shadow-sm p-6 animate-in fade-in slide-in-from-top-4">
              <h2 className="text-xl font-semibold mb-6">
                {editingId ? t("newSource.editSource") : t("newSource.addSource")}
              </h2>
                ...
                {formData.source_type === "link" ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      />
                      <p className="text-xs text-muted-foreground">{t("newSource.fileHelp")}</p>
                    </div>
                    {formData.content && (
                      <div className="mt-2">
                        <label className="text-xs font-medium mb-1 block">
                          {t("newSource.previewContent")}
                        </label>
                        <textarea
                          value={formData.content}
                          readOnly
          ...
          {!loading && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredSources.map((source) => (
                <div
                  key={source.id}
                  className="group bg-card border rounded-lg p-5 shadow-sm hover:shadow-md transition-all relative"
                >
                  {/* Favorite Button */}
                  <button
                    ...
                    title={
                      favorites.includes(source.id)
                        ? t("newSource.removeFavorite")
                        : t("newSource.addFavorite")
                    }
                  >
                    <Heart
                      className={`w-4 h-4 ${favorites.includes(source.id) ? "fill-current" : ""}`}
                    />
                  </button>
                  ...
                  <div className="flex justify-between items-start mb-3 pr-8">
                    <div className="flex items-center gap-2 min-w-0 overflow-hidden flex-1 mr-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="font-semibold truncate" title={source.name}>
                          {source.name}
                        </h3>
                  ...
                  <div className="flex flex-wrap gap-1 mb-2">
                    {source.metadata?.tags &&
                      ...
                      source.metadata.tags.map((tagId: any) => {
