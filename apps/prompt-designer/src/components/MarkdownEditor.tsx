    ...
    switch (action) {
      case "bold":
        newText =
          ...
          `**${selectedText || t("markdownEditor.sample.bold")}**` +
          value.substring(end);
      ...
      case "italic":
        newText =
          value.substring(0, start) +
          `*${selectedText || t("markdownEditor.sample.italic")}*` +
          value.substring(end);
      ...
      case "strikethrough":
        newText =
          value.substring(0, start) +
          `~~${selectedText || t("markdownEditor.sample.strikethrough")}~~` +
          value.substring(end);
        newCursorPos = start + 2;
        break;
      case "heading1":
        newText =
          value.substring(0, start) +
          `# ${selectedText || t("markdownEditor.sample.heading1")}` +
          value.substring(end);
        newCursorPos = start + 2;
        break;
      case "heading2":
        newText =
          value.substring(0, start) +
          `## ${selectedText || t("markdownEditor.sample.heading2")}` +
          value.substring(end);
      ...
      case "heading3":
        newText =
          value.substring(0, start) +
          `### ${selectedText || t("markdownEditor.sample.heading3")}` +
          value.substring(end);
      ...
      case "bulletList":
        newText =
          value.substring(0, start) +
          `- ${selectedText || t("markdownEditor.sample.bullet")}` +
          value.substring(end);
        newCursorPos = start + 2;
        break;
      case "orderedList":
        newText =
          value.substring(0, start) +
          `1. ${selectedText || t("markdownEditor.sample.ordered")}` +
          value.substring(end);
        newCursorPos = start + 3;
        break;
      case "codeBlock":
      ...
      case "blockquote":
        newText =
          value.substring(0, start) +
          `> ${selectedText || t("markdownEditor.sample.quote")}` +
          value.substring(end);
        newCursorPos = start + 2;
        break;
      case "link":
        newText =
          value.substring(0, start) +
          `[${selectedText || t("markdownEditor.sample.link")}](url)` +
          value.substring(end);
        newCursorPos = start + 1;
        break;
      case "image":
