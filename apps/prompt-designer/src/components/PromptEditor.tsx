...
const normalizeToolChanges = (
  changes: ToolState | undefined,
...
const defaultVariables = ["prospect_country", "prospect_city", "prospect_name", "phone_number"];
...
const defaultVariablesObjs: PromptVariable[] = defaultVariables.map((v) =>
  toPromptVariable(v, "System")
);
...
export const PromptEditor: React.FC<PromptEditorProps> = ({ promptId, onBack }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
          ...
          try {
            ...
            loadedVars.forEach((vName: string) => {
              if (!existingMap.has(vName)) {
                existingMap.set(
                  vName,
                  toPromptVariable(vName, t("promptEditor.localVariableType"))
                );
              }
            });
            ...
            const loadedAuthor = availableAuthors.find(
              (a) => a.name === loadedData.metadata.author
            );
            if (loadedAuthor) {
        ...
        // If no promptId, start fresh — but check for injected prompt from www via query param
        if (!promptId) {
          localStorage.removeItem(STORAGE_KEY);

          let injectedRaw: string | null = null;
          try {
            const params = new URLSearchParams(window.location.search);
            const encoded = params.get("pd");
            if (encoded) {
              injectedRaw = decodeURIComponent(escape(atob(decodeURIComponent(encoded))));
          ...
          if (injectedRaw) {
            try {
              const injected = JSON.parse(injectedRaw) as {
                title?: string;
                description?: string;
                content?: string;
                source?: string;
              ...
              const injectedData: PromptData = {
                ...buildDefaultPromptData(toolsCatalog),
                metadata: {
                  version: "1.0.0",
                  author: "",
                  lastModified: new Date().toISOString(),
                  ...
                  tags: injected.source ? [injected.source] : [],
                },
                sections: {
                  ...createEmptySections(),
                  identity_role: injected.content || "",
                },
              };

              setBaselineData(injectedData);
              setPromptData(injectedData);

              const titles: { [key: string]: string } = {};
              Object.keys(injectedData.sections).forEach((key) => {
                titles[key] = resolveSectionTitle(key, t);
              });
              setSectionTitles(titles);
              setIsInitialized(true);
              ...
              toast({
                title: t("promptEditor.injectedTitle"),
                description: t("promptEditor.injectedDescription", {
                  title: injected.title || "Prompt",
                }),
              });
              return;
            } catch {
              // ignore invalid injection data
            }
          }
        ...
        try {
          ...
          if (parsed?.current) {
            ...
            const migratedMetadata = {
              ...
              title:
                rawMetadata.title ||
                (rawMetadata as any).description ||
                t("promptEditor.restoredTitle"),
              description: rawMetadata.description || "",
              tags:
                rawMetadata.tags ||
                ((rawMetadata as any).promptType ? [(rawMetadata as any).promptType] : []),
            };
            ...
            const customSections = Object.keys(mergedSections).filter(
  ...
  const currentTools = useMemo(
    () => normalizeToolState(promptData.tools, availableTools),
    [promptData.tools, availableTools]
  );
  const enabledTools = useMemo(
    () => availableTools.filter((tool) => currentTools[tool.name]),
  ...
  const updateMetadata = useCallback(
    <K extends keyof PromptData["metadata"]>(field: K, value: PromptData["metadata"][K]) => {
      setPromptData((prev) => ({
        ...prev,
        metadata: {
          ...prev.metadata,
  ...
  const removeTag = (tag: string) => {
    updateMetadata(
      "tags",
      promptData.metadata.tags.filter((t) => t !== tag)
    );
  };
  ...
  const handleConsolidatedChange = useCallback(
    (content: string) => {
      ...
      toast({
        title: t("promptEditor.saved"),
        description: t("promptEditor.editConsolidatedNotice"),
      });
    },
    [t, toast]
  );
  ...
  const addVariable = useCallback(
    (variable: string) => {
      ...
      if (variable && !variables.some((v) => v.variable_name === variable)) {
        setVariables((prev) => [
          ...prev,
          toPromptVariable(variable, t("promptEditor.customVariableType")),
        ]);
        toast({
  ...
  const addNewSection = useCallback(() => {
    ...
    if (sectionName?.trim()) {
      ...
      const sectionKey = cleanName
        .toLowerCase()
        .replace(/\s+/g, "_")
        .replace(/[^a-z0-9_]/g, ""); // Schema keys use underscores mostly
      ...
      if (!Object.prototype.hasOwnProperty.call(promptData.sections, sectionKey)) {
        ...
        toast({
          ...
          description: t("promptEditor.sectionExists"),
          variant: "destructive",
        });
      }
    }
    ...
    (sectionKey: string) => {
      if (confirm(t("promptEditor.sectionDeleteConfirm", { title: getSectionTitle(sectionKey) }))) {
        ...
        toast({
          ...
          description: t("promptEditor.sectionDeletedDescription", {
  ...
  const handleSaveVersion = useCallback(async () => {
    ...
    if (!authorIdToSave) {
      toast({
        title: t("promptEditor.error"),
        description: t("promptEditor.authorRequired"),
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ current: promptData, history: [...history, entry] })
      );
    } catch (e: any) {
  ...
  const handleCopy = () => {
    ...
    toast({ title: t("promptEditor.copy") });
  };
  const handleImportPrompt = (e: any) => {}; // Placeholder

  return (
    <div className="h-screen flex flex-col bg-editor-background relative overflow-hidden">
      {isLoading && (
      ...
      {/* Header */}
      <header className="shrink-0 bg-card border-b border-editor-border px-4 sm:px-6 py-3">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0 overflow-x-auto">
            {onBack && (
              <button
                onClick={onBack}
                disabled={isLoading}
                className="flex items-center gap-1 sm:gap-2 text-muted-foreground hover:text-foreground shrink-0"
              >
                <ArrowLeft size={20} />
                <span className="text-sm hidden sm:inline">{t("promptEditor.projects")}</span>
              </button>
            )}
            <Badge variant="secondary" className="shrink-0">
              v{promptData.metadata.version}
            </Badge>
            {promptData.metadata.tags.map((t) => (
              <Badge key={t} variant="outline" className="shrink-0 hidden sm:inline-flex">
                {t.toUpperCase()}
              </Badge>
            ))}
          </div>
          {currentPromptId && (
            ...
            >
              <Trash size={16} />{" "}
              <span className="hidden sm:inline">{t("promptEditor.delete")}</span>
            </Button>
          )}
        </div>
      </header>
      ...
      <div className="flex flex-1 min-h-0">
        <VariablePanel
          ...
          onInsertVariable={(variableName) => {
            ...
            if (editor) {
              ...
              toast({
                title: t("promptEditor.error"),
                description: t("promptEditor.selectSectionError"),
                variant: "destructive",
              });
            }
          }}
        ...
        <div className="flex-1 min-h-0 min-w-0 overflow-y-auto">
          {/* Metadata Card */}
          <Card className="m-2 sm:m-4 p-3 sm:p-4">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {selectedTagIds.length === 0 ? (
                        <span className="text-muted-foreground">
                          {t("promptEditor.tagsPlaceholder")}
                        </span>
                      ) : (
                  ...
                  <SelectContent>
                    {availableAuthors.map((author) => (
                      <SelectItem key={author.id} value={author.name}>
                        {author.name}
                        {author.team && (
                          <span className="text-muted-foreground ml-2">({author.team})</span>
                        )}
                      </SelectItem>
                    ))}
          ...
          {/* Sections Grid */}
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4 p-2 sm:p-4 items-start">
            {/* Render Default Sections First */}
            {DEFAULT_SECTION_KEYS.map((key) => (
              <div
                key={key}
                className="w-full h-[300px] sm:h-[400px] lg:h-[500px] flex-shrink-0 relative group"
              >
            ...
            <div
              className="w-full h-[200px] sm:h-[300px] lg:h-[500px] flex items-center justify-center border-2 border-dashed border-muted rounded-lg hover:bg-accent/50 transition-colors cursor-pointer"
