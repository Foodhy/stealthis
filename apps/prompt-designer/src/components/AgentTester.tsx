...
export const AgentTester: React.FC<AgentTesterProps> = ({ isOpen, onClose, systemTemplate, variables }) => {
  ...
  const loadOllamaModels = React.useCallback(async () => {
    try {
      ...
      if (!res.ok) throw new Error('No se pudieron obtener los modelos de Ollama');
      ...
      toast({ title: 'Ollama', description: `${err?.message || 'Error al listar modelos'} (${ollamaBaseUrl})`, variant: 'destructive' });
    }
  }, [model, toast, ollamaBaseUrl]);
  ...
  const sendMessage = async () => {
    ...
    if (provider === 'ollama' && !currentModel) {
      toast({ title: 'Modelo requerido', description: 'Selecciona un modelo de Ollama', variant: 'destructive' });
      return;
    }
    if (provider === 'openai' && (!openaiApiKey || !currentModel)) {
      toast({ title: 'Configuración incompleta', description: 'Proporciona API key y modelo', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      if (provider === 'ollama') {
        ...
        if (!res.ok) throw new Error('Fallo la generación con Ollama');
        const data = await res.json();
        ...
        if (!res.ok) {
          const msg = data?.error?.message || data?.message || 'Fallo la generación con el proveedor OpenAI compatible';
          throw new Error(msg);
        }
      ...
      <DialogContent className="max-w-6xl h-[90vh] flex flex-col">
        <DialogHeader>
          <div className="flex items-center justify-between w-full">
            <DialogTitle className="text-xl font-semibold">Probar Agente</DialogTitle>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 overflow-hidden">
          <div className="lg:col-span-2 flex flex-col min-h-0">
            <Card className="flex-1 overflow-hidden flex flex-col">
              <div className="p-3 border-b flex items-center gap-3">
                ...
                {provider === 'ollama' ? (
                  <>
                    <Input className="w-[240px]" placeholder="Base URL Ollama" value={ollamaBaseUrl} onChange={(e) => setOllamaBaseUrl(e.target.value)} />
                    ...
                    <Button variant="ghost" size="sm" onClick={loadOllamaModels}>Refrescar modelos</Button>
                  </>
                ) : (
                  <div className="flex items-center gap-2 flex-wrap">
                    <Input className="w-[220px]" placeholder="Base URL" value={openaiBaseUrl} onChange={(e) => setOpenaiBaseUrl(e.target.value)} />
                    ...
                    <Input className="w-[200px]" placeholder="Modelo" value={model} onChange={(e) => setModel(e.target.value)} />
                  </div>
                )}
              </div>
              ...
              <ScrollArea className="flex-1">
                <div className="p-4 space-y-3">
                  <div className="text-xs text-muted-foreground">
                    ...
                    <Badge variant="outline">Prompt consolidado</Badge>
                  </div>
                  <Separator />
                  {messages.length === 0 && (
                    <div className="text-sm text-muted-foreground">Inicia la conversación. Usa Cmd/Ctrl+Enter para enviar.</div>
                  )}
                  {messages.map((m, idx) => (
                    <div key={idx} className="text-sm">
                      <span className="font-medium mr-2">{m.role === 'user' ? 'Tú' : 'Agente'}:</span>
              ...
              <div className="p-3 border-t">
                <div className="flex items-end gap-2">
                  <Textarea value={messageInput} onChange={(e) => setMessageInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Escribe un mensaje... (Cmd/Ctrl+Enter)" className="flex-1" />
                  <Button onClick={sendMessage} disabled={isSending || !messageInput.trim()} className="min-w-[96px] flex items-center justify-center gap-2">
                    {isSending ? (<><Loader2 className="h-4 w-4 animate-spin" /><span>Enviando…</span></>) : 'Enviar'}
                  </Button>
                </div>
              </div>
            </Card>
          ...
          <div className="lg:col-span-1 flex flex-col min-h-0">
            <Card className="flex-1 overflow-hidden">
              <div className="p-3 border-b text-sm font-medium">Variables</div>
              <ScrollArea className="h-full">
                <div className="p-3 space-y-3">
                  {detectedVariables.length === 0 && (
                    <div className="text-xs text-muted-foreground">No se detectaron variables en el prompt.</div>
                  )}
