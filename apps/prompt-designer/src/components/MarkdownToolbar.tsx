...
export const MarkdownToolbar: React.FC<MarkdownToolbarProps> = ({
  ...
  return (
    <div className="flex items-center gap-1 p-2 border-b border-border bg-card/50">
      {/* Headings */}
      <div className="flex items-center gap-1">{renderActionGroup(defaultActions)}</div>

      <Separator orientation="vertical" className="h-6" />

      {/* Text Formatting */}
      <div className="flex items-center gap-1">{renderActionGroup(formattingActions)}</div>

      <Separator orientation="vertical" className="h-6" />

      {/* Lists */}
      <div className="flex items-center gap-1">{renderActionGroup(listActions)}</div>

      <Separator orientation="vertical" className="h-6" />

      {/* Insert Elements */}
      <div className="flex items-center gap-1">{renderActionGroup(insertActions)}</div>

      <Separator orientation="vertical" className="h-6" />

      {/* Alignment */}
      <div className="flex items-center gap-1">{renderActionGroup(alignActions)}</div>

      <Separator orientation="vertical" className="h-6" />

      {/* History */}
      <div className="flex items-center gap-1">{renderActionGroup(historyActions)}</div>

      {/* Custom Actions */}
      {customActions.length > 0 && (
        <>
          <Separator orientation="vertical" className="h-6" />
          <div className="flex items-center gap-1">{renderActionGroup(customActions)}</div>
        </>
      )}
    </div>
  );
