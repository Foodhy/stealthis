import type { BuilderGeneratedFile, BuilderOverwriteStrategy } from "./contracts";
...
interface WriteOptions {
  ...
  overwrite: BuilderOverwriteStrategy;
  confirmOverwrite?: (path: string) => boolean;
}
...
export async function writeFilesToDirectory(options: WriteOptions): Promise<WriteSummary> {
  ...
  for (const file of options.files) {
    const { dirs, fileName } = splitPath(file.path);
    const targetDirectory = await getOrCreateDirectory(options.directory, dirs);
    const exists = await fileExists(targetDirectory, fileName);
    ...
    if (exists) {
      ...
      if (options.overwrite === "ask") {
        const decision = options.confirmOverwrite
          ? options.confirmOverwrite(file.path)
          : window.confirm(`Replace existing file ${file.path}?`);
      ...
      summary.replaced.push(file.path);
    }

    await writeTextFile(targetDirectory, fileName, file.content);
...
export function createZipBlob(files: BuilderGeneratedFile[]): Blob {
...
export function downloadZipFallback(
  files: BuilderGeneratedFile[],
  fileName = "stealthis-project.zip"
): void {
