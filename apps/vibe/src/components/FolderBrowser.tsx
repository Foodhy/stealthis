    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
      <div className="w-full max-w-lg rounded-xl border border-white/8 bg-slate-900 shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-white/8 px-4 py-3">
          <h2 className="text-sm font-medium text-slate-200">Select project folder</h2>
        ...
        <div className="flex items-center gap-2 border-b border-white/6 px-4 py-2">
          ...
          <span className="flex-1 truncate font-mono text-xs text-slate-300">{currentPath}</span>
        </div>

        {/* Directory list */}
        <div className="max-h-64 overflow-y-auto px-2 py-2">
          {loading && (
            <div className="px-2 py-4 text-center text-xs text-slate-500">Loading...</div>
          )}
          {error && <div className="px-2 py-2 text-xs text-red-400">{error}</div>}
          {!loading && dirs.length === 0 && !error && (
            <div className="px-2 py-4 text-center text-xs text-slate-500">No subdirectories</div>
          )}
          {dirs.map((name) => (
            <button
              key={name}
